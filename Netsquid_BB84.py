...
def protocol(a):
    ...
    channel_a2b = CombinedChannel("QC_Channel_a2b", length=a, models={"delay_model": FibreDelayModel(), "quantum_loss_model": FibreLossModel(p_loss_init=0, p_loss_length=0.2), "quantum_noise_model": DepolarNoiseModel(depolar_rate=1e3, time_independent=False)}, transmit_empty_items=True)
    channel_b2a = CombinedChannel("QC_Channel_b2a", length=a, models={"delay_model": FibreDelayModel(), "quantum_loss_model": FibreLossModel(p_loss_init=0, p_loss_length=0.2), "quantum_noise_model": DepolarNoiseModel(depolar_rate=1e3, time_independent=False)}, transmit_empty_items=True)
...
if __name__ == "__main__":
	...
	key_bit_error = np.ones(100)
	for a in range(100):
		protocol(a)
		x[a] = a
	
	plt.title("Key bit error vs Quantum channel lenth")
	plt.xlabel("channel lenth")
	plt.ylabel("Key bit error rate")
	plt.plot(x, key_bit_error)
	plt.show()
